...
const journey_access_1 = require("./journey.access");
const loyalty_service_1 = require("../loyalty/loyalty.service");
const notification_service_1 = require("../notifications/notification.service");
const airport_repo_1 = require("../airports/airport.repo");
const journey_repo_1 = require("./journey.repo");
const airport_repo_2 = require("../airports/airport.repo");
const navigation_1 = require("../airports/navigation");
const StepCatalog = [
...
class JourneyService {
    ...
    getJourney(userId, accountNumber) {
        const access = (0, journey_access_1.getJourneyAccess)(userId, accountNumber);
        ...
        let journey = (0, journey_repo_1.findActiveJourneyForUser)(userId);
        if (!journey || journey.is_demo !== (access.type === 'demo' ? 1 : 0)) {
            if (!journey) {
                const airport = (0, airport_repo_1.findAirportByCode)(access.airportCode);
                if (!airport)
                    return { access: access.type, journey: null };
                journey = {
                    id: (0, ids_1.genId)('jny'),
                    user_id: userId,
                ...
                (0, journey_repo_1.insertJourney)(journey);
                this.buildSteps(journey.id, 'CAI', access.type === 'personal');
            }
            journey = (0, journey_repo_1.findActiveJourneyForUser)(userId);
        }
        const steps = (0, journey_repo_1.stepsForJourney)(journey.id).map((s) => this.decorateStep(s, access));
    ...
    buildSteps(journeyId, _airportCode, isPersonal) {
        const nodes = (0, airport_repo_2.nodesForAirport)((0, airport_repo_1.findAirportByCode)('CAI').id);
        StepCatalog.forEach((spec, idx) => {
            const node = nodes.find((n) => n.type === spec.type) || null;
            const step = {
                id: (0, ids_1.genId)('jst'),
            ...
            (0, journey_repo_1.insertJourneyStep)(step);
        });
    }
    decorateStep(step, access) {
        let navigation = null;
        let route = null;
        const nodes = (0, airport_repo_2.nodesForAirport)((0, airport_repo_1.findAirportByCode)('CAI').id);
        ...
        if (node) {
            ...
            if (entrance && node.id !== entrance.id) {
                const r = (0, navigation_1.findRoute)('CAI', entrance.id, node.id, nodes);
                if (r.found)
                    route = { distance: r.distance, walkingTime: r.walkingTime, route: r.nodes };
            }
        }
        return {
            id: step.id,
    ...
    completeStep(userId, journeyId, stepId) {
        const journey = (0, journey_repo_1.findJourneyById)(journeyId);
        if (!journey)
            throw (0, errors_1.notFound)('Journey not found');
        if (journey.user_id !== userId)
            throw (0, errors_1.forbidden)('You do not own this journey');
        let step = (0, journey_repo_1.findStepById)(journeyId, stepId);
        if (!step)
            throw (0, errors_1.notFound)('Step not found');
        const accountRow = (0, connection_1.getDb)().prepare('SELECT account_number FROM users WHERE id = ?').get(userId);
        const accountNumber = accountRow ? accountRow.account_number : 0;
        if (step.status === 'completed') {
            // idempotent
            return this.getJourney(userId, accountNumber);
        }
        const steps = (0, journey_repo_1.stepsForJourney)(journeyId);
        const stepIndex = steps.findIndex((s) => s.id === stepId);
        if (stepIndex < 0)
            throw (0, errors_1.badRequest)('Step not in journey');
        (0, journey_repo_1.markStepCompleted)(stepId);
        const updatedSteps = (0, journey_repo_1.stepsForJourney)(journeyId);
        ...
        const user = (0, connection_1.getDb)().prepare('SELECT account_number, loyalty_points FROM users WHERE id = ?').get(userId);
        ...
        if (allComplete) {
            (0, journey_repo_1.updateJourneyProgress)(journey, updatedSteps.length - 1, 100, 'completed');
            ...
            if (newCurrent < updatedSteps.length) {
                (0, journey_repo_1.markStepStatus)(updatedSteps[newCurrent].id, 'current');
            }
            // any upcoming beyond that stay upcoming
            for (let i = newCurrent + 1; i < updatedSteps.length; i++) {
                if (updatedSteps[i].status === 'current')
                    (0, journey_repo_1.markStepStatus)(updatedSteps[i].id, 'upcoming');
            }
            (0, journey_repo_1.updateJourneyProgress)(journey, newCurrent, progress, 'in_progress');
            const pct = progress;
            if (pct >= 60) {
    ...
    getJourneyById(userId, journeyId, accountNumber) {
        const journey = (0, journey_repo_1.findJourneyById)(journeyId);
        if (!journey)
            throw (0, errors_1.notFound)('Journey not found');
        if (journey.user_id !== userId)
